...
- `@earendil-works/pi-ai` stays in `dependencies`, not `peerDependencies`: `pi install` runs npm with `--legacy-peer-deps` and `--omit=dev`, so only `dependencies` reach an installed extension. That subpath is also internal to pi-ai, so a pi-ai bump that drops it fails `npm run typecheck`.
